...
use kuzmiand\users\components\AbstractItemEvent;
use Yii;
use yii\base\Event;
...
abstract class AbstractItem extends ActiveRecord
{
	...
	public static function removeChildren($parentName, $childrenNames)
	{
		...
		foreach ($childrenNames as $childName)
		{
			Yii::$app->db->createCommand()
				->delete(Yii::$app->getModule('user')->auth_item_child_table, ['parent' => $parentName, 'child' => $childName])
				->execute();
		}
	...
	{
		return Yii::$app->getModule('user')->auth_item_table;
	}
	...
	{
		return parent::find()->andWhere([Yii::$app->getModule('user')->auth_item_table . '.type'=>static::ITEM_TYPE]);
	}
